...
function get_bordrou_filtered_communes_action() {
    if ( !isset($_POST['wilaya_id']) || !isset($_POST['shipping_method_id']) ) {
        wp_send_json_error(array('message' => 'Missing parameters.'));
        return;
    }

    $wilaya_id = sanitize_text_field($_POST['wilaya_id']);
    $shipping_method_id = sanitize_text_field($_POST['shipping_method_id']); // e.g., local_pickup:138 or local_pickup_dhd:123

    // --- Dynamically Determine Provider ---
    $provider_slug = null;
    $instance_id = null;
    $selected_shipping_method_obj = null;

    // Extract instance ID
    $method_id_parts = explode(':', $shipping_method_id);
    if (count($method_id_parts) == 2) {
        $instance_id = (int) $method_id_parts[1];
    } else {
         wp_send_json_error(array('message' => 'Invalid shipping_method_id format.'));
         return;
    }

    // Find the shipping method object based on instance ID
    $shipping_zones = \WC_Shipping_Zones::get_zones();
    foreach ($shipping_zones as $zone) {
        $shipping_methods = $zone['shipping_methods'];
        foreach ($shipping_methods as $shipping_method_obj) {
            if ($shipping_method_obj->instance_id == $instance_id) {
                $selected_shipping_method_obj = $shipping_method_obj;
                break 2; // Found it
            }
        }
    }
    // Check default zone (zone 0) if not found yet
    if (!$selected_shipping_method_obj) {
         $zone0_methods = \WC_Shipping_Zones::get_zone(0)->get_shipping_methods();
         foreach ($zone0_methods as $shipping_method_obj) {
             if ($shipping_method_obj->instance_id == $instance_id) {
                 $selected_shipping_method_obj = $shipping_method_obj;
                 break; 
             }
    ...
    if (!$selected_shipping_method_obj) {
        wp_send_json_error(array('message' => 'Could not find shipping method object for instance ID: ' . $instance_id));
        return;
    }

    // Determine provider slug
    if (method_exists($selected_shipping_method_obj, 'get_provider')) {
        $provider_slug = $selected_shipping_method_obj->get_provider();
    } 
    // Fallback: Try to guess from class name if get_provider doesn't exist (less reliable)
    else {
        $shipping_method_class = get_class($selected_shipping_method_obj);
        // Example guessing logic (needs refinement based on Bordrou's actual class names)
        if (strpos(strtolower($shipping_method_class), 'yalidine') !== false) $provider_slug = 'yalidine';
        elseif (strpos(strtolower($shipping_method_class), 'dhd') !== false) $provider_slug = 'dhd'; // Add guesses for other providers
        elseif (strpos(strtolower($shipping_method_class), 'picosolution') !== false) $provider_slug = 'picosolution'; 
        // ... add more guesses based on observed class names ...
    }

    if (!$provider_slug) {
         wp_send_json_error(array('message' => 'Could not determine Bordrou provider for the selected shipping method.'));
         return;
    }
    // --- End Dynamic Provider Determination ---


    // Check required Bordrou classes exist
    if ( !class_exists('\WooBordereauGenerator\Admin\Partials\BordereauGeneratorProviders') ) {
        wp_send_json_error(array('message' => 'Bordrou Generator Providers class not found. Is the plugin active?'));
        return;
    }
     $providers = \WooBordereauGenerator\Admin\Partials\BordereauGeneratorProviders::get_providers();
     if (!isset($providers[$provider_slug])) {
         wp_send_json_error(array('message' => 'Provider details not found in Bordrou settings for: ' . $provider_slug));
         return;
     }
     $provider_details = $providers[$provider_slug];
     $settings_class_name = $provider_details['setting_class'] ?? null; // Get class name from provider data

     if (empty($settings_class_name) || !class_exists($settings_class_name)) {
          wp_send_json_error(array('message' => 'Bordrou provider settings class (' . $settings_class_name . ') not found for ' . $provider_slug));
         return;
     }


    // --- Check for API Credentials ---
    $api_key_option = $provider_slug . '_api_key'; // Assuming standard option naming
    $api_token_option = $provider_slug . '_api_token'; // Assuming standard option naming
    // Some providers might use different option names - adjust if needed
    if ( empty(get_option($api_key_option)) /* Add || empty(get_option($api_token_option)) if token is always required */ ) {
         // Log the check failure but don't necessarily stop, as some providers might not need keys/tokens stored this way
         error_log('Bordrou Debug: API credentials options (' . $api_key_option . ') might be missing for provider: ' . $provider_slug);
         // wp_send_json_error(array('message' => 'Bordrou Generator API credentials for ' . $provider_slug . ' might be missing in settings.'));
         // return; // Decide if this should be a fatal error
    }
    // --- End Check ---

    try {
       
        $provider_settings = new $settings_class_name($provider_details);

        if (!method_exists($provider_settings, 'get_communes')) {
            // If get_communes doesn't exist, maybe this provider doesn't support filtering? Return empty success.
             error_log('Bordrou Debug: get_communes method not found in ' . $settings_class_name . '. Returning empty list.');
             wp_send_json_success(array()); // Send empty data
            return;
        }

        // Call get_communes with $hasStopDesk = true
        // Ensure wilaya_id is passed in the format expected by the specific provider's get_communes method
        // (Yalidine removes 'DZ-', others might need it or just the number)
        // For now, assume removing 'DZ-' is common, but this might need provider-specific logic
        $wilaya_id_for_provider = str_replace('DZ-', '', $wilaya_id); 
        
        $filtered_communes = $provider_settings->get_communes($wilaya_id_for_provider, true);

        // --- DEBUGGING ---
        if (is_wp_error($filtered_communes)) {
            wp_send_json_error(array('message' => 'Error fetching communes: ' . $filtered_communes->get_error_message()));
        } else {
            wp_send_json_success($filtered_communes);
        }
