<?php
function napoleon_get_social_networks() {
	return apply_filters( 'napoleon_social_networks', array(
		array(
			'name'  => 'facebook',
			'label' => esc_html__( 'Facebook', 'napoleon' ),
			'icon'  => 'fab fa-facebook-f',
		),
		array(
			'name'  => 'twitter',
			'label' => esc_html__( 'Twitter', 'napoleon' ),
			'icon'  => 'fab fa-twitter',
		),
		array(
			'name'  => 'instagram',
			'label' => esc_html__( 'Instagram', 'napoleon' ),
			'icon'  => 'fab fa-instagram',
		),
		array(
			'name'  => 'tiktok',
			'label' => esc_html__( 'Tiktok', 'napoleon' ),
			'icon'  => 'fab fa-tiktok',
		),
		array(
			'name'  => 'snapchat',
			'label' => esc_html__( 'Snapchat', 'napoleon' ),
			'icon'  => 'fab fa-snapchat',
		),
		array(
			'name'  => 'bloglovin',
			'label' => esc_html__( 'Bloglovin', 'napoleon' ),
			'icon'  => 'fas fa-heart',
		),
		array(
			'name'  => 'pinterest',
			'label' => esc_html__( 'Pinterest', 'napoleon' ),
			'icon'  => 'fab fa-pinterest',
		),
		array(
			'name'  => 'youtube',
			'label' => esc_html__( 'YouTube', 'napoleon' ),
			'icon'  => 'fab fa-youtube',
		),
		array(
			'name'  => 'vimeo',
			'label' => esc_html__( 'Vimeo', 'napoleon' ),
			'icon'  => 'fab fa-vimeo',
		),
		array(
			'name'  => 'linkedin',
			'label' => esc_html__( 'LinkedIn', 'napoleon' ),
			'icon'  => 'fab fa-linkedin',
		),
		array(
			'name'  => 'tumblr',
			'label' => esc_html__( 'Tumblr', 'napoleon' ),
			'icon'  => 'fab fa-tumblr',
		),
		array(
			'name'  => 'flickr',
			'label' => esc_html__( 'Flickr', 'napoleon' ),
			'icon'  => 'fab fa-flickr',
		),
		array(
			'name'  => 'dribbble',
			'label' => esc_html__( 'Dribbble', 'napoleon' ),
			'icon'  => 'fab fa-dribbble',
		),
		array(
			'name'  => 'wordpress',
			'label' => esc_html__( 'WordPress', 'napoleon' ),
			'icon'  => 'fab fa-wordpress',
		),
		array(
			'name'  => '500px',
			'label' => esc_html__( '500px', 'napoleon' ),
			'icon'  => 'fab fa-500px',
		),
		array(
			'name'  => 'soundcloud',
			'label' => esc_html__( 'Soundcloud', 'napoleon' ),
			'icon'  => 'fab fa-soundcloud',
		),
		array(
			'name'  => 'spotify',
			'label' => esc_html__( 'Spotify', 'napoleon' ),
			'icon'  => 'fab fa-spotify',
		),
		array(
			'name'  => 'vine',
			'label' => esc_html__( 'Vine', 'napoleon' ),
			'icon'  => 'fab fa-vine',
		),
		array(
			'name'  => 'tripadvisor',
			'label' => esc_html__( 'Trip Advisor', 'napoleon' ),
			'icon'  => 'fab fa-tripadvisor',
		),
		array(
			'name'  => 'telegram',
			'label' => esc_html__( 'Telegram', 'napoleon' ),
			'icon'  => 'fab fa-telegram',
		),
	) );
}

function napoleon_wp_link_pages_default_args() {
	return apply_filters( 'napoleon_wp_link_pages_default_args', array(
		'before'      => '<div class="page-links">' . esc_html__( 'Pages:', 'napoleon' ),
		'after'       => '</div>',
 ...
 */
function napoleon_get_related_posts( $post_id, $related_count, $args = array() ) {
 ...
 */
function napoleon_get_page_var( $default_return = 0 ) {
 ...
 */
function napoleon_dropdown_posts( $args = '', $name = 'post_id' ) {
	$defaults = array(
		'depth'                 => 0,
		'post_parent'           => 0,
		'selected'              => 0,
	...
	$output = apply_filters( 'napoleon_dropdown_posts', $output, $name, $r );
 ...
 */
function napoleon_color_luminance( $color, $percent ) {
	// Remove # if provided
	if ( '#' === $color[0] ) {
	...
	for ( $i = 0; $i < 3; $i ++ ) {
		...
		// Explicitly round the float result before converting to hex to avoid implicit conversion notice.
		$new_hex .= str_pad( dechex( round( $dec ) ), 2, 0, STR_PAD_LEFT ); 
	}
 ...
 * @return string
 */
function napoleon_hex2rgba( $color, $opacity = false ) {
...
function napoleon_empty_content( $content = false ) {
	if ( false === $content ) {
		$post    = get_post();
 ...
 * @return string
 */
function napoleon_get_image_lightbox_caption( $image_id = false ) {
	if ( false === $image_id ) {
	...
	if ( is_array( $attachment ) ) {
		$field = apply_filters( 'napoleon_image_lightbox_caption_field', 'caption', $image_id, $attachment );
 ...
 */
function napoleon_head_preconnect_google_fonts() {
	?>
